...
tape('/ (GET) - options displayed correctly for primary and secondary: --> ' + __filename, function (t) {
  sessions.addAll(function () {
    init(config, function (error, server, pool) {
      t.ok(!error, 'No error on init server');
      server.inject(landingPage('primary_3'), function (res) {
        t.equal(res.payload.split('<li class="landing__card"').length - 1, 4, 'primary offered the correct number of activities/options');
        ...
        server.inject(landingPage('secondary_12'), function (res) {
          t.equal(res.payload.split('<li class="landing__card"').length - 1, 2, 'secondary offered the correct number of activities/options');
